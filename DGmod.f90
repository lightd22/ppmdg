...
MODULE DGmod
    ...
    ! ########################################################################
        SUBROUTINE Cmat(N,nodes,wghts,dx,dxelem,output)
            ...
            DO i=0,N
                DO k=0,N
                    DO m=0,N
                        xi = dz*(i+0.5D0*(nodes(m)+1))-1D0
                        HOLDER(m) = wghts(m)*phi(xi,k,N,nodes)
                    END DO
                    output(i,k) = SUM(HOLDER)*0.5D0
                END DO
            END DO
    ...
    ! #############################################################################
        ...
        REAL(KIND = DOUBLE) FUNCTION phitld(x,N,nodes,j,Ain,nelems)
            REAL(KIND = DOUBLE), INTENT(IN) :: x
            INTEGER, INTENT(IN) :: j, N,nelems
            REAL(KIND=DOUBLE), DIMENSION(0:N,0:nelems+1), INTENT(IN) :: Ain
            REAL(KIND=DOUBLE), DIMENSION(0:N), INTENT(IN) :: nodes
			REAL(KIND=DOUBLE), DIMENSION(0:N) :: HOLDER
            INTEGER :: i
            ...
            DO i = 0, N
                 HOLDER(i) = Ain(i,j)*phi(x,i,N,nodes)
            END DO
			phitld = SUM(HOLDER)
